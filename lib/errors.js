...
const ERR_NULLS_NOT_ALLOWED = 175;
const ERR_DPL_TOO_MUCH_DATA = 176;
const ERR_DPL_INVALID_ROW_DATA = 177;
const ERR_PIPELINE_OPERATION_NOT_SUPPORTED = 178;
const ERR_PIPELINE_MISMATCHED_TOKEN = 179;
const ERR_PIPELINE_DBOBJECTS_NOT_SUPPORTED = 180;
const ERR_PIPELINE_LARGE_BUFFERS_NOT_SUPPORTED = 181;
const ERR_PIPELINE_EXEC_OPTION_NOT_SUPPORTED = 182;
const ERR_INVALID_SQL_NAME = 183;
const ERR_APP_CONTEXT_EMPTY_NAMESPACE = 184;
const ERR_APP_CONTEXT_INVALID_KEY_VALUE = 185;
const ERR_APP_CONTEXT_KEY_TOO_LONG = 186;
const ERR_APP_CONTEXT_VALUE_TOO_LONG = 187;
const ERR_UNSUPPORTED_KEY_LENGTH = 188;
const ERR_INVALID_END_USER_SECURITY_CONTEXT_LENGTH = 189;
const ERR_END_USER_SECURITY_CONTEXT_REQUIRES_TCPS = 190;
const ERR_UNSUPPORTED_DEEP_DATA_SECURITY_FEATURE = 191;
const ERR_CHAR_CS_NOT_SUPPORTED = 192;
const ERR_INVALID_PROPERTY_LENGTH = 193;
...
const ERR_CONNECT_DESCRIPTOR_LIMIT_EXCEEDED = 522;
...
const ERR_TOO_MANY_REDIRECTS = 536;
const ERR_ANO_INTEGRITY = 537;
...
adjustErrorXref.set("ORA-24315", ERR_APP_CONTEXT_EMPTY_NAMESPACE);
...
messages.set(ERR_NOT_IMPLEMENTED,                       // NJS-089
  '%s is not supported by node-oracledb in %s mode');
...
messages.set(ERR_NULLS_NOT_ALLOWED,                     // NJS-175
  'Value for column %s may not be null on row %d');
messages.set(ERR_DPL_TOO_MUCH_DATA,                     // NJS-176
  'The maximum size of a Direct Path load has been exceeded');
messages.set(ERR_DPL_INVALID_ROW_DATA,                  // NJS-177
  'The row data provided for Direct Path load is Invalid. Expected a 2D array (Array of Arrays) representing rows and columns.');
messages.set(ERR_PIPELINE_OPERATION_NOT_SUPPORTED,      // NJS-178
  'Unsupported pipeline operation type: %s');
messages.set(ERR_PIPELINE_MISMATCHED_TOKEN,             // NJS-179
  'Pipeline internal error: token number %d does not match expected token number %d');
messages.set(ERR_PIPELINE_DBOBJECTS_NOT_SUPPORTED,      // NJS-180
  'Oracle Database Object types are not supported in pipeline mode');
messages.set(ERR_PIPELINE_LARGE_BUFFERS_NOT_SUPPORTED,  // NJS-181
  'PL/SQL blocks with string/bytes object size greater than 32k are not supported in pipeline mode');
messages.set(ERR_PIPELINE_EXEC_OPTION_NOT_SUPPORTED,    // NJS-182
  `Execute option '%s' is not supported in pipeline mode`);
messages.set(ERR_INVALID_SQL_NAME,                     // NJS-183
  'invalid SQL name: embedded double quotes are not allowed');
messages.set(ERR_APP_CONTEXT_EMPTY_NAMESPACE,           // NJS-184
  'The namespace can not be empty for Application Context');
messages.set(ERR_APP_CONTEXT_INVALID_KEY_VALUE,         // NJS-185
  'Invalid Application Context entry: both key and value must be strings. Got key=%s, value=%s');
messages.set(ERR_APP_CONTEXT_KEY_TOO_LONG,              // NJS-186
  'Application Context key length (%d bytes) exceeds the maximum allowed (%d bytes)');
messages.set(ERR_APP_CONTEXT_VALUE_TOO_LONG,            // NJS-187
  'Application Context value length (%d bytes) exceeds the maximum allowed (%d bytes)');
messages.set(ERR_UNSUPPORTED_KEY_LENGTH,               // NJS-188
  'Unsupported encryption key length %d');
messages.set(ERR_INVALID_END_USER_SECURITY_CONTEXT_LENGTH, // NJS-189
  'Specified End User Context exceeds the maximum supported size %d');
messages.set(ERR_END_USER_SECURITY_CONTEXT_REQUIRES_TCPS,  // NJS-190
  'End User Context requires use of the tcps protocol');
messages.set(ERR_UNSUPPORTED_DEEP_DATA_SECURITY_FEATURE,   // NJS-191
  'database version does not support the Oracle Deep Data Security feature');
messages.set(ERR_CHAR_CS_NOT_SUPPORTED,                 // NJS-192
  'Character set id %d is not supported by node-oracledb in Thin mode');
messages.set(ERR_INVALID_PROPERTY_LENGTH,               // NJS-193
  'The length of the parameter %s exceeds the maximum allowed length of %d');
...
messages.set(ERR_CONNECT_DESCRIPTOR_LIMIT_EXCEEDED,     // NJS-522
  'connection string exceeds supported descriptor %s limit of %d');
...
messages.set(ERR_ANO_NEGOTIATION,                       // NJS-533
  'Advanced Networking Option service negotiation failed.\nCause: ORA-%s');
...
messages.set(ERR_TOO_MANY_REDIRECTS,               // NJS-536
  'Too many connect redirects (limit: %d)');
messages.set(ERR_ANO_INTEGRITY,                    // NJS-537
  'Crypto-checksum mismatch on a packet received from the server');
...
//
// Asserts that the property value of a parameter is a non-negative integer
// value (or undefined).
//-----------------------------------------------------------------------------
...
// assertPropLength()
//
// Asserts that the property value is of specified type and doesn't exceed
// maximum specified length
//-----------------------------------------------------------------------------
function assertPropLength(value, propType, propMaxLength, propName) {
  if (propType !== 'array')
    assertPropValue(typeof value === propType, propName);
  else
    assertPropValue(Array.isArray(value), propName);
  assert(value.length <= propMaxLength, ERR_INVALID_PROPERTY_LENGTH, propName,
    propMaxLength);
}
...
function getErr(errorNum) {
  ...
  if (errorNum === ERR_INVALID_PACKET && args.length > 1) {
    baseText += '. Cause: %s';
  }
...
// throwNotImplemented()
//
// Throws an error that the feature is not supported in Thin/Thick mode
//-----------------------------------------------------------------------------
function throwNotImplemented(feature) {
  const settings = require('./settings.js');
  throwErr(ERR_NOT_IMPLEMENTED, feature, settings.thin ? 'Thin' : 'Thick');
}
...
module.exports = {
  ...
  ERR_CONNECTION_EOF,
  ERR_CONNECT_DESCRIPTOR_LIMIT_EXCEEDED,
  ...
  ERR_DPL_INVALID_ROW_DATA,
  ERR_APP_CONTEXT_INVALID_KEY_VALUE,
  ERR_APP_CONTEXT_KEY_TOO_LONG,
  ERR_APP_CONTEXT_VALUE_TOO_LONG,
  ERR_UNSUPPORTED_KEY_LENGTH,
  ERR_APP_CONTEXT_EMPTY_NAMESPACE,
  ...
  ERR_ACCESS_TOKEN,
  ERR_CALLOUT_FN,
  ERR_PIPELINE_OPERATION_NOT_SUPPORTED,
  ERR_PIPELINE_MISMATCHED_TOKEN,
  ERR_PIPELINE_DBOBJECTS_NOT_SUPPORTED,
  ERR_PIPELINE_LARGE_BUFFERS_NOT_SUPPORTED,
  ERR_PIPELINE_EXEC_OPTION_NOT_SUPPORTED,
  ERR_INVALID_SQL_NAME,
  ERR_CHAR_CS_NOT_SUPPORTED,
