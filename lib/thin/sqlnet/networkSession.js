...
const { resolveConnectStr } = require('../../impl/parserHelpers.js');
...
function getRedirectParams(params) {
  return Object.fromEntries(
    Object.entries(params).filter(([key]) => !REDIRECT_SECURITY_KEYS.has(key))
  );
}
...
class NetworkSession {
  ...
  async transportConnect(address) {
    ...
    const effectiveHttpsProxy = address?.httpsProxy || this.sAtts?.nt?.httpsProxy;

    // Enforce TCPS whenever an HTTPS proxy would be used (either at the address
    // level or via the session-global proxy fallback in NTTCP).
    if (protocol === 'TCP' && effectiveHttpsProxy) {
  ...
  async connect2(address, userConfig) {
    ...
    /* Read the response */
      ...
      if (packet.type === constants.NSPTRF) { /* REFUSE */
        ...
        if (redirectCount >= MAX_REDIRECTS) {
          errors.throwErr(errors.ERR_TOO_MANY_REDIRECTS, MAX_REDIRECTS);
        }
        redirectCount++;
        ...
        /* Add on attributes from redirect connect String */
        if (address.desc?.params) {
          this.sAtts.setFrom(getRedirectParams(address.desc.params));
        }
    ...
    if (!this.sAtts.noNA) {
      ...
      const keyBuf = NAContext.publicKeyPacket();
      if (keyBuf) {
        this._send(keyBuf, 0, keyBuf.length);
        this.flush();
      }
      NAContext.startServices();
      this._startANO(NAContext.cryptAlgo, NAContext.hashAlgo);
    }
  ...
  _processPacket(packet) {
    switch (packet.type) {
      case constants.NSPTDA: { /* DATA packet */
        if (this.cryptAlgo || this.hashAlgo) {
          this._unprotectPacket(packet);
        }
        ...
        if ((dataFlags & constants.NSPDAFCMP) != 0) { // compression enabled
          if (!this.sAtts.networkCompressionEnabled) {
            errors.throwErr(errors.ERR_INVALID_PACKET, "Compressed packet received without negotiation");
          }
          ...
          const maxPayload = this.sAtts.largeSDU
            ? Math.min(this.sAtts.sdu, constants.NSPABSSDULN)
            : Math.min(this.sAtts.sdu, constants.NSPMXSDULN);

          const inflateOptions = {
            finishFlush: zlib.constants.Z_SYNC_FLUSH,
            maxOutputLength: maxPayload
          };
          try {
            if (this.sAtts.firstRecvCompressedPacket) {
              deCompressedDataBuffer = zlib.inflateSync(buffertoDeCompress, inflateOptions);
              this.sAtts.firstRecvCompressedPacket = 0;
            } else {
              deCompressedDataBuffer = zlib.inflateRawSync(buffertoDeCompress, inflateOptions);
            }
          } catch (err) {
            if (err.code == 'ERR_BUFFER_TOO_LARGE') {
              errors.throwErr(errors.ERR_INVALID_PACKET,
                `Decompressed packet exceeds SDU ${maxPayload}`);
            }
            errors.throwErr(errors.ERR_DATA_COMPRESSION, err.message);
          }
          const resultLength = deCompressedDataBuffer.length;
          if (resultLength > maxPayload) {
            errors.throwErr(errors.ERR_INVALID_PACKET, `Decompressed size ${resultLength} exceeds SDU ${maxPayload}`);
          }
  ...
  /**
   * Turns on the negotiated algorithms and reserves the room their framing
   * adds to every data packet, so that a full packet still fits the SDU.
   */
  _startANO(cryptAlgo, hashAlgo) {
    this.cryptAlgo = cryptAlgo;
    this.hashAlgo = hashAlgo;
    if (!cryptAlgo && !hashAlgo) {
      return;
    }
    this.anoOverhead = 1; /* folding key */
    if (hashAlgo) {
      this.anoOverhead += hashAlgo.size;
    }
    if (cryptAlgo) {
      this.anoOverhead += 16 + 1; /* worst case padding, plus its count */
    }
  }

  _writeLength(buf) {
    if (this.sAtts.largeSDU) {
      buf.writeUInt32BE(buf.length, constants.NSPHDLEN);
    } else {
      buf.writeUInt16BE(buf.length, constants.NSPHDLEN);
    }
  }

  /**
   * Checksums then encrypts the payload of an outgoing data packet. The 10
   * byte header stays in clear text; the server reads the length from it.
   */
  _protectPacket(buf) {
    let payload = buf.subarray(constants.NSPDADAT);
    if (this.hashAlgo) {
      payload = Buffer.concat([payload, this.hashAlgo.compute(payload)]);
    }
    if (this.cryptAlgo) {
      payload = this.cryptAlgo.encrypt(payload);
    }
    const out = Buffer.concat([buf.subarray(0, constants.NSPDADAT), payload,
      Buffer.from([0])]);
    this._writeLength(out);
    return out;
  }

  _unprotectPacket(packet) {
    if (packet.buf.length - constants.NSPDADAT <= 1) {
      return;
    }
    /* The folding key sits outside what the digest covers. */
    let payload = packet.buf.subarray(constants.NSPDADAT, packet.buf.length - 1);
    if (this.cryptAlgo) {
      payload = this.cryptAlgo.decrypt(payload);
    }
    if (this.hashAlgo) {
      payload = this.hashAlgo.validate(payload);
    }
    packet.buf = Buffer.concat([packet.buf.subarray(0, constants.NSPDADAT),
      payload]);
    this._writeLength(packet.buf);
  }
  ...
  async reset() {
    ...
    /* The checksum chain counts packets, and a reset drops whatever the peer
       had in flight. Both ends restart it here or every later packet fails. */
    if (this.hashAlgo) {
      this.hashAlgo.init();
    }
