...
/* 11.2.0.2. Announcing a later version makes the server derive session keys
   by a scheme this driver does not implement. */
const ANO_SERVICE_VERSION = 0xB200200;

/* Algorithm ids as the server numbers them, mapped to the key size and digest
   they select. Offered best first, preceded by id 0 so that a server which
   does not require ANO can still answer "none" and stay in clear text. */
const ENCRYPTION_ALGOS = new Map([[17, 32], [16, 24], [15, 16]]);
const INTEGRITY_ALGOS = new Map([[5, 'sha256'], [6, 'sha384'], [4, 'sha512']]);

/* Subpacket count that marks a data integrity reply carrying a DH group. */
const DH_SUBPACKETS = 8;
...
class NetworkService {
  ...
  sendVersion(buf, pos) {
    ...
    pos = buf.writeUInt32BE(this.version, pos);
    return pos;
  }
  ...
  receiveRaw(pkt, ret, pos) {
    const len = [];
    pos = this.receiveType(pkt, RAW_TYPE, len, pos);
    ret[0] = pkt.subarray(pos, pos + len[0]);
    return pos + len[0];
  }
...
class EncryptionService extends NetworkService {
  ...
  constructor(sAtts) {
    ...
    this.version = ANO_SERVICE_VERSION;
    this.numSubPackets = 3;
    this.algos = Buffer.from([0, ...ENCRYPTION_ALGOS.keys()]);
    this.algoId = 0;
  }

  getDataSize() {
    return  SERVICE_HEADER_SIZE + SUBPACKET_SIZE + VERSION_LENGTH +
            SUBPACKET_SIZE + this.algos.length + SUBPACKET_SIZE + 1;
  }

  sendServiceData(buf, pos) {
    pos = this.sendHeader(buf, pos);
    pos = this.sendVersion(buf, pos);
    pos = this.sendRaw(buf, this.algos, this.algos.length, pos);
    pos = this.sendUB1(buf, 1, pos); /* number of encryption drivers */
    return pos;
  }

  receiveServiceData(pkt, numSubPackets, pos) {
    const ret = [];
    pos = this.receiveType(pkt, VERSION_TYPE, ret, pos);
    pos = this.receiveType(pkt, UB1_TYPE, ret, pos);
    this.algoId = ret[0];
    return pos;
  }

  /**
   * @param {Buffer} sessionKey shared secret from the data integrity service
   * @returns {PayloadCipher} null when the server chose no encryption
   */
  activate(sessionKey) {
    if (this.algoId == 0) {
      return null;
    }
    const keyLength = ENCRYPTION_ALGOS.get(this.algoId);
    if (!keyLength || !sessionKey) {
      errors.throwErr(errors.ERR_ANO_STATUS, "Encryption");
    }
    return new PayloadCipher(sessionKey.subarray(0, keyLength));
  }
}
...
class DataIntegrityService extends NetworkService {
  ...
  constructor(sAtts) {
    ...
    this.algos = Buffer.from([0, ...INTEGRITY_ALGOS.keys()]);
    this.algoId = 0;
  }

  getDataSize() {
    return   SERVICE_HEADER_SIZE + SUBPACKET_SIZE + VERSION_LENGTH +
            SUBPACKET_SIZE + this.algos.length;
  }
  ...
  receiveServiceData(pkt, numSubPackets, pos) {
    ...
    pos = this.receiveType(pkt, UB1_TYPE, ret, pos);
    this.algoId = ret[0];
    if (numSubPackets != DH_SUBPACKETS) {
      return pos;
    }

    pos = this.receiveType(pkt, UB2_TYPE, ret, pos);
    const length = Math.ceil(ret[0] / 8);
    pos = this.receiveType(pkt, UB2_TYPE, ret, pos); /* prime bit length */
    const gen = [], prime = [], serverKey = [], iv = [];
    pos = this.receiveRaw(pkt, gen, pos);
    pos = this.receiveRaw(pkt, prime, pos);
    pos = this.receiveRaw(pkt, serverKey, pos);
    pos = this.receiveRaw(pkt, iv, pos);
    if (prime[0].length != length || serverKey[0].length != length) {
      errors.throwErr(errors.ERR_ANO_PACKET);
    }

    const keys = diffieHellman(gen[0], prime[0], serverKey[0], length);
  ...
  /**
   * @returns {DataIntegrityHash} null when the server chose no checksumming
   */
  activate() {
    if (this.algoId == 0) {
      return null;
    }
    const algorithm = INTEGRITY_ALGOS.get(this.algoId);
    if (!algorithm || !this.sessionKey) {
      errors.throwErr(errors.ERR_ANO_STATUS, "Data integrity");
    }
    return new DataIntegrityHash(algorithm, this.sessionKey, this.iv);
  }
}
...
class ANO {
  ...
  /**
   * Builds the follow-up packet handing the client's Diffie-Hellman public key
   * to the server. Must be sent before any algorithm is activated.
   *
   * @returns {Buffer} null when the server negotiated no key exchange
   */
  publicKeyPacket() {
    const publicKey = this.serviceList[2].publicKey;
    if (!publicKey) {
      return null;
    }
    const bufSize = HEADER_SIZE + SERVICE_HEADER_SIZE + SUBPACKET_SIZE +
                    publicKey.length;
    const buf = Buffer.allocUnsafe(bufSize).fill(0);
    let pos = buf.writeUInt32BE(MAGIC);
    pos = buf.writeUInt16BE(bufSize, pos);
    ...
    pos = buf.writeUInt16BE(DATAINTEGRITY, pos);
    pos = buf.writeUInt16BE(1, pos);
    pos = buf.writeUInt32BE(0, pos);
    this.serviceList[2].sendRaw(buf, publicKey, publicKey.length, pos);

    return buf;
  }

  /**
   * Turns on the negotiated algorithms. Data integrity goes first because it
   * owns the shared secret the cipher keys itself with.
   */
  startServices() {
    this.hashAlgo = this.serviceList[2].activate();
    this.cryptAlgo = this.serviceList[1].activate(this.serviceList[2].sessionKey);
  }
}
