...
class AuthMessage extends Message {
  ...
  constructor(conn, config) {
    ...
    this.allowLegacyVerifier = !!config.allowLegacyVerifier;
  ...
  getAlterTimezoneStatement() {
    ...
    if (process.env.ORA_SDTZ) {
      ...
      // LOCAL and DBTIMEZONE are SQL keywords and must not be quoted.
      if (["LOCAL", "DBTIMEZONE"].includes(tzRepr.toUpperCase())) {
    ...
    return `ALTER SESSION SET TIME_ZONE =${parser.enquoteLiteral(tzRepr)}\x00`;
  }

  encode(buf) {
    let verifier11G = false;
    let verifier10G = false;
      ...
      if (this.changePassword) {
        ...
        if (this.externalAuth) {
          ...
          this.verifierType === constants.TNS_VERIFIER_TYPE_11G_2) {
            verifier11G = true;
          } else if (this.verifierType === constants.TNS_VERIFIER_TYPE_10G &&
              this.allowLegacyVerifier) {
            verifier10G = true;
          ...
          const serverCompileCaps = buf.caps.serverCompileCaps;
          const supportsO7Logon = !!(serverCompileCaps &&
            serverCompileCaps[constants.TNS_CCAP_LOGON_TYPES] & constants.TNS_CCAP_O7LOGON);
          ED.updateVerifierData(this.sessionData, this.password, this.newPassword, verifier11G, this, verifier10G, supportsO7Logon);
