...
const O3LOGON_FIXED_KEY = Buffer.from([1, 35, 69, 103, 137, 171, 205, 239]);
const ZERO_DES_IV = Buffer.alloc(8);

function o3logonExtend(str) {
  const upper = str.toUpperCase();
  const buf = Buffer.alloc(upper.length * 2);
  for (let i = 0; i < upper.length; i++) {
    buf[i * 2] = 0;
    buf[i * 2 + 1] = upper.charCodeAt(i);
  }
  return buf;
}

function desCbcLastBlock(input, key) {
  const cbc = DES.CBC.instantiate(DES.DES);
  const enc = cbc.create({ type: 'encrypt', key, iv: ZERO_DES_IV, padding: false });
  const out = Buffer.from(enc.update(input).concat(enc.final()));
  return out.subarray(out.length - 8);
}

function deriveO3LogonKey(username, password) {
  let buffer = Buffer.concat([o3logonExtend(username), o3logonExtend(password)]);
  const remainder = buffer.length % 8;
  if (remainder > 0) {
    buffer = Buffer.concat([buffer, Buffer.alloc(8 - remainder)]);
  }
  const key1 = desCbcLastBlock(buffer, O3LOGON_FIXED_KEY);
  const key2 = desCbcLastBlock(buffer, key1);
  return Buffer.concat([key2, Buffer.alloc(8)]);
}
...
class EncryptDecrypt {
   ...
   */
  updateVerifierData(sessionData, password, newPassword, verifier11G, authObj, verifier10G, supportsO7Logon) {
    let keyLen = 32;
    let hashAlg = 'sha512';
    ...
    if (verifier10G) {
      keyLen = 16;
      passwordHash = deriveO3LogonKey(authObj.username, password);
    } else if (verifier11G) {
      keyLen = 24;
      hashAlg = 'sha1';
    ...
    if (verifier10G && supportsO7Logon) {
      authObj.sessionKey = encodedClientKey.slice().toString('hex').toUpperCase()
        .slice(0, sessionKeyPartb.length * 2);
      ...
      for (let i = 0; i < 16; i++) {
        buf[i] = sessionKeyParta[i + 16] ^ sessionKeyPartb[i + 16];
      }
      authObj.comboKey = crypto.createHash("md5").update(buf).digest();
    } else if (sessionKeyParta.length === 48) {
