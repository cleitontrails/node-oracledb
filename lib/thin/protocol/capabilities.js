...
class Capabilities {
  ...
  constructor(nscon) {
    ...
    this.charsetId = constants.TNS_CHARSET_UTF8;
    ...
    this.supportsEndUserSecurityContext = false;
  }

  adjustForServerCompileCaps(serverCaps, nscon) {
    this.serverCompileCaps = serverCaps;
    ...
    const bpIndex = constants.TNS_CCAP_FEATURE_BACKPORT2;
    if (bpIndex < serverCaps.length &&
        (serverCaps[bpIndex] & constants.TNS_CCAP_END_USER_SEC_CTX_PIGGYBACK)) {
      this.supportsEndUserSecurityContext = true;
    }
  }
  ...
  initCompileCaps(nscon) {
    ...
    this.compileCaps[constants.TNS_CCAP_TTC5] =
      ...
      constants.TNS_CCAP_TOKEN_SUPPORTED |
      constants.TNS_CCAP_PIPELINING_SUPPORT |
      constants.TNS_CCAP_PIPELINING_BREAK |
    ...
    this.compileCaps[constants.TNS_CCAP_FEATURE_BACKPORT2] = constants.TNS_CCAP_END_USER_SEC_CTX_PIGGYBACK;
  }
  ...
  checkCharsetId() {
    if (this.charsetId !== constants.TNS_CHARSET_UTF8)
      errors.throwErr(errors.ERR_CHAR_CS_NOT_SUPPORTED, this.charsetId);
  }
