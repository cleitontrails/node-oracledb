...
class ReadPacket extends BaseBuffer {
  ...
  /**
   * It is used in pipeline mode to ignore reading packets
   * if client detects a protocol error.
   */
  resetPackets() {
    this.savedPackets = [];
    this.savedPacketPos = 0;
  ...
  /**
   * Discards the remaining responses after the pipeline has
   * encountered an exception in ABORT_ON_ERROR mode
   */
  async discardPipelineResponses(num) {
    while (num > 0) {
      await this.waitForPackets(true);
      num--;
    }
    this.resetPackets();
  }

  _isEndOfResponse(packet) {
    // In pipeline mode, when a DML operation fails due to invalid RETURNING
    // binds, we may receive a partial response followed by a break packet
    // before the full response arrives. Such break packets are ignored.
    //
    // In non-pipeline mode, return true if the packet is not a data packet.
    if (packet.type !== constants.TNS_PACKET_TYPE_DATA) {
      return !this.nsi.pipelineEnabled;
    }

    // End-of-response marker
    if (packet.buf.readUInt16BE(8) &
    constants.TNS_DATA_FLAGS_END_OF_RESPONSE) {
      return true;
    }

    // Single-byte 1D packet
    return (
      packet.buf.length === MSG_TYPE_OFFSET &&
      packet.buf[MSG_TYPE_OFFSET - 1] === constants.TNS_MSG_TYPE_END_OF_REQUEST
    );
  }
  ...
  async waitForPackets(checkRequestBoundary = false) {
    ...
    if (checkRequestBoundary && this.nsi.endOfRequestSupport) {
      while (!this._isEndOfResponse(packet)) {
        if (
          packet.type !== constants.TNS_PACKET_TYPE_DATA &&
          this.nsi.isBreak
        ) {
          // For pipeline Mode only, we issue a dummy reset to the sqlnet layer
          // to set state, `this.isBreak = this.isReset = false;`
...
class WritePacket extends BaseBuffer {
  ...
  constructor(nsi, caps, protocol) {
    super(nsi.sAtts.sdu - nsi.anoOverhead);
  ...
  _asBuffer(input) {
    ...
    errors.throwErr(errors.ERR_INTERNAL, 'Expected Buffer or string');
  }

  // Writes a 32-bit length, writeUB4 followed by the field if present.
  // For Null input, We just write length as 0.
  _writeBytesWithTwoLengths(input) {
    if (input == null) {
      this.writeUB4(0);
      return;
    }
    const buf = this._asBuffer(input);
    const length = buf.length;
    this.writeUB4(length);
    if (length > 0) {
  ...
  _sendPacket(finalPacket = false) {
    ...
    this.writeUInt8(0);
    this.writeUInt16BE(0);
    let flags = this._dataFlags;
    let buf = this.buf.subarray(0, size);
    if (!finalPacket) {
      buf = Buffer.from(buf);
      this.startPacket();
    } else {
      if (this.nsi.pipelineEnabled) {
        // For pipeline mode, next operation would start writing
        // into this.buf. Hence create a copy so that it wont
        // overwrite the contents given to stream.write().
        buf = Buffer.from(buf);
      }
      // Write End of RPC bit in last packet used only for fastAuth Message
      // and in pipeline mode.
      flags |= constants.TNS_DATA_FLAGS_END_OF_REQUEST;
    }
    if (!this.nsi.ntAdapter) {
      errors.throwErr(errors.ERR_INVALID_CONNECTION);
    }
    if (this.packetType === constants.TNS_PACKET_TYPE_DATA) {
      buf.writeUInt16BE(flags, constants.TNS_DATA_FLAGS_OFFSET);
    }
  ...
  startPacket() {
    ...
    if (this.packetType === constants.TNS_PACKET_TYPE_DATA) {
      this.writeUInt16BE(this._dataFlags);
    }
  }
  ...
  writeStrKeywordValuePair(key, valueBytes, flags = 0, text) {
    this.writeUB4(flags);
    this._writeBytesWithTwoLengths(key);
    this._writeBytesWithTwoLengths(text);
    this._writeBytesWithTwoLengths(valueBytes);
  }
