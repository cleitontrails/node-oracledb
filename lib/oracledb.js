...
const parserHelpers = require('./impl/parserHelpers.js');
...
const EndUserSecurityContext = require('./endUserSecurityContext.js');
...
// validate external authentication credentials for token and non-token Authentications
function _validateExternalAuthCredentials(outOptions, inCreatePool, isTokenAuth = false) {
  if (!outOptions.externalAuth) {
    return;
  }
  if (outOptions.password !== undefined) {
    errors.throwErr(isTokenAuth ? errors.ERR_TOKEN_BASED_AUTH
      : errors.ERR_WRONG_CRED_FOR_EXTAUTH);
  }
  if (outOptions.user) {
    if (inCreatePool && !settings.thin) {
      // Thick mode currently does not allow proxy users to be specified
...
async function _verifyOptions(options, inCreatePool) {
  ...
  if (options.allowLegacyVerifier !== undefined) {
    errors.assertParamPropValue(typeof options.allowLegacyVerifier === 'boolean', 1,
      "allowLegacyVerifier");
    outOptions.allowLegacyVerifier = options.allowLegacyVerifier;
  }
  ...
  if (options.accessToken !== undefined) {
    _validateExternalAuthCredentials(outOptions, inCreatePool, true);
  ...
  if (outOptions.token === undefined && outOptions.externalAuth) {
    _validateExternalAuthCredentials(outOptions, inCreatePool);
  }
...
async function createPool(options) {
  ...
  options = await _updateUserOptionsFromConnectString(options, true);
  ...
  try {
    // registers a callback poolEventReg having 'eventName' and 'arguments of the
    // event' as its arguments. It is passed as part of options while creating impl
    // pool, so that events emitted in impl pool can access the lib pool instance.
    // For now, no argument is passed as args, but can be used later for future metrics.
    options.poolEventReg = ((eventName, ...args) => {
      if (traceHandler.isEnabled()) {
        traceHandler.getTraceInstance()?.[eventName](pool, ...args);
      }
    });
...
async function getConnection(a1) {
  ...
  options = await _updateUserOptionsFromConnectString(options, false);
  settings.addToOptions(options,
    "connectionClass",
    "driverName",
    "edition",
...
async function _setConfigParameters(obj, credential, configProvider) {
  ...
  const configObject = params ? { ...params } : {};
...
async function _retrieveParamValueFromVault(paramObj, credential, configProvider) {
  ...
  if (paramObj.type == "base64") {
    ...
    if (configProvider == 'azurevault' || configProvider == 'ocivault' || configProvider == 'awssecretsmanager') {
      console.log("WARNING: Plain Text in a JSON Password should only be used in development environments");
      return paramObj.value;
    } else
      errors.throwErr(errors.ERR_CONFIG_PROVIDER_PARAM_TYPE, 'password type text is only allowed in ocivault, azurevault and awssecretsmanager');
...
async function _checkConfigProvider(options) {
  ...
  if (configProviderCache && (cachedConfigEntry = configProviderCache.get(url))) {
    ...
    if ((Date.now() - cachedConfigEntry.timeAdded) < cacheEntriesDuration) {
      ...
      if (secondOpts.password instanceof ObfuscatedValue) {
        secondOpts.password = secondOpts.password.get();
      }

      //deobfuscate walletContent
      if (secondOpts.walletContent instanceof ObfuscatedValue) {
        secondOpts.walletContent = secondOpts.walletContent.get();
      }
    }
  }
  if (!secondOpts) {
    ...
    if (match) {
      ...
      // second being the credential to the configProvider
        ...
        if (secondOpts.password)
          cacheOpts.password = new ObfuscatedValue(secondOpts.password);
        if (secondOpts.walletContent)
          cacheOpts.walletContent = new ObfuscatedValue(secondOpts.walletContent);
...
async function _updateUserOptionsFromConnectString(options, inCreatePool) {
  let tempOptions;
  const hasConnectString = options.connectString || options.connectionString;
  if (!settings.thickModeDSNPassthrough && hasConnectString) {
    tempOptions = Object.create(Object.getPrototypeOf(options));
    Object.defineProperties(tempOptions, Object.getOwnPropertyDescriptors(options));
    const { connectString, resolver } = parserHelpers.getConnectDescriptorFromEZConnect(tempOptions.connectString || tempOptions.connectionString);
    if (tempOptions.connectString !== undefined) {
      tempOptions.connectString = connectString;
    } else if (tempOptions.connectionString != undefined) {
      tempOptions.connectionString = connectString;
    }

    if (resolver) parserHelpers.mergeExtendedParams(tempOptions, resolver);
  } else {
    tempOptions = options;
  }
  options = await _verifyOptions(tempOptions, inCreatePool);
  if (!settings.thickModeDSNPassthrough && options.connectString) {
    await parserHelpers.applyConnectStringOverrides(options);
  }
  return options;
}
...
function enquoteLiteral(value) {
  errors.assertArgCount(arguments, 1, 1);
  errors.assertParamValue(typeof value === 'string', 1);
  return parser.enquoteLiteral(value);
}

//-----------------------------------------------------------------------------
// enquoteName()
//
// Returns a quoted SQL identifier. Embedded double quotes are rejected and the
// value is converted to uppercase using locale-independent Unicode rules when
// capitalize is true.
//-----------------------------------------------------------------------------
function enquoteName(name, capitalize = true) {
  errors.assertArgCount(arguments, 1, 2);
  errors.assertParamValue(typeof name === 'string', 1);
  if (arguments.length > 1)
    errors.assertParamValue(typeof capitalize === 'boolean', 2);

  if (name.includes('"')) {
    errors.throwErr(errors.ERR_INVALID_SQL_NAME);
  }
  return `"${capitalize ? name.toUpperCase() : name}"`;
}

//-----------------------------------------------------------------------------
// isQualifiedSqlName()
//
// Returns whether the trimmed value is a qualified SQL name. Qualified names
// are dotted simple SQL names with an optional trailing @ database link, and
// the database link name can itself be dotted.
//-----------------------------------------------------------------------------
function isQualifiedSqlName(name) {
  errors.assertArgCount(arguments, 1, 1);
  errors.assertParamValue(typeof name === 'string', 1);
  const text = name.trim();
  return parser.parseQualifiedSqlName(text, 0) === text.length;
}

//-----------------------------------------------------------------------------
// isSimpleSqlName()
//
// Returns whether the trimmed value is a single simple SQL name.
//-----------------------------------------------------------------------------
function isSimpleSqlName(name) {
  errors.assertArgCount(arguments, 1, 1);
  errors.assertParamValue(typeof name === 'string', 1);
  const text = name.trim();
  return parser.parseSimpleSqlName(text, 0) === text.length;
}
...
module.exports = {
  ...
  get thickModeDSNPassthrough() {
    return settings.thickModeDSNPassthrough;
  },

  set thickModeDSNPassthrough(value) {
    errors.assertPropValue(typeof value === 'boolean', "thickModeDSNPassthrough");
    settings.thickModeDSNPassthrough = value;
  },
};
